...
\label{eq:FT3}
\end{equation}
E.q.~\ref{eq:FT2} therefore means that while the power spectrum remains unchanged for shifted signals, i.e. $\mid \hat{h}(\xi)\mid ^2 = \mid\hat{f}(\xi)\mid^2$, an extra phase $\Delta \phi(\xi)$ is added because of the additional term $e^{-2 \pi ix_0 \xi}$: 
\begin{equation}
...
An intuitive, but equally quantitative way, to see how E.q.\ref{eq:PhaseShift} holds is as follows. The Fourier transform decomposes the function $f(x)$ into a frequency representation $\hat{f}(\xi)$, each frequency component accompanied by the orthogonal basis $e^{2 \pi ix \xi}$, as in the form of inverse Fourier transform: 
\begin{equation}
...
\label{eq:gradient}
\end{equation}
where $\Delta$ is used to refer to the phase difference between a shifted line profile and an unshifted~/~referenced line profile, while the derivative refers to the response of $\Delta \phi$ to $\xi$. $\phi(\xi)$ can be obtained by extracting the phase information from the Fourier transform of line profiles. After that, a simple linear regression model can be fit to a plot of $\Delta \phi(\xi)$ versus $\xi$ (Fig.~\ref{fig:FT}) to enable measurement of the bulk shift between two line profiles $x_0$. 
