...
In this case, the input radial velocities would be the apparent radial velocities of deformed line profiles (also known as jitter). Both velocities $RV_\text{FT}$ and $RV_\text{Gaussian}$ are plotted against rotation phase (Fig.~\ref{fig:rv_recovery_deformed}). If we take the root-mean-squares of both $RV_\text{FT}$ and $RV_\text{Gaussian}$ to be the intrinsic noise level ($rms_\text{FT} = rms_\text{Gaussian} = 0.08$ m/s) corresponding S/N = 10,000, the uncertainty of $RV_\text{FT} - RV_\text{Gaussian}$ as residual would have an uncertainty of 
$\sqrt{rms_\text{FT}^2+rms_\text{Gaussian}^2}\approx0.11$ m/s. As $\mid RV_\text{difference}\mid = \mid RV_\text{FT} - RV_\text{Gaussian}\mid < 0.03$ m/s, we can agree that $RV_\text{FT}$ and $RV_\text{Gaussian}$ are indistinguishably consistent. 
...
We can therefore write the following measurable quantities -- $RV_\text{Gaussian}$, $RV_\text{FT,L}$ and $RV_\text{FT,H}$ --
...
\end{align}
Note that only (any) two of them are independent equations, as each of the three equations above one can be derived from the other two (by subtraction). Rearranging yields two out of three independent expressions for the jitter model
% We retain all of them regardless to be discussed in the following.
\begin{align*}
...
\end{align*}
where $RV_\text{Gaussian}, RV_\text{FT,L}$ and $RV_\text{FT,H}$ are direct measurements, whereas $k_L$ and $k_H$ are unknowns\footnote{We could determine $k_L$ and $k_H$ in the demonstrated simulations only because we knew the jitter.} but can be incorporated into the radial velocity models (i.e. substitute any of Eq.~\ref{eq:jitter_model1}, Eq.~\ref{eq:jitter_model2} and Eq.~\ref{eq:jitter_model3} into the $RV_\text{jitter}$ in Eq.~\ref{eq:RV_Gau}) in the process of recovering planets.
